...
<!--        </v-col>-->
        ...
        <v-col cols="6" md="8" lg="10">
          ...
          <v-list-item-content>
            <v-list-item-title v-html="item.charName"></v-list-item-title>
      ...
      <v-sheet v-if="showTimeout" outlined rounded>
        ...
        <v-card-actions>
          ...
          <v-btn v-if="selectedChar && selectedChar.charId" @click="newWindow" color="primary"><v-icon left>mdi-account</v-icon>케릭터로 이동</v-btn>
        </v-card-actions>
      </v-sheet>
        ...
        <div class="mx-auto text-center">
          ...
          <template v-else>
            <v-avatar tile class="mr-2" size="100" >
              <img :src="selectedChar.profileImg"
          ...
          <v-btn v-if="selectedChar.legionId" text outlined class="mr-2 mt-1" @click="showLegion">
            <v-icon left>mdi-shield-half-full</v-icon>
            {{selectedChar.legionName}}
          </v-btn>
          </template>
...
export default {
  ...
  mounted() {
    ...
    this.selectedServer = localStorage.getItem("server");
  ...
  watch: {
    ...
    selectedChar () {
      this.retry = 0;
      console.info(this.selectedChar)
      if(this.selectedChar && this.selectedChar.charId){
        this.selectedChar.charName = this.selectedChar.charName.replace(/(<([^>]+)>)/ig, '')
  ...
  methods: {
    async search(keyword) {
      ...
      try{
        ...
        const response = await this.axios.get(`/search/v1/characters?classId=&pageNo=1&pageSize=50&query=${keyword}&raceId=&serverId=${id}`, {
          timeout: 2000,
          cancelToken: this.cancelSource.token
        });
        if (response && response.data) {
          this.suggest = response.data.documents;
    ...
    async findChar() {
      ...
      this.showServerError = false;
      console.info(this.selectedChar)
      const {serverId, charId} = this.selectedChar;

      try{
        const data = {"keyList":["character_stats","character_equipments","character_abyss","character_stigma"]};
        // const response = await this.axios.put(`/game/v2/classic/merge/server/${}/id/${}`, data, {
        const response = await this.axios.put(`/game/v2/classic/merge/server/${serverId}/id/${charId}`, data);
    ...
    newWindow(){
      const {serverId, charId} = this.selectedChar;
      window.open(`https://aion.plaync.com/characters/server/${serverId}/id/${charId}/home`);
      // this.showTimeout = false;
    },
    showLegion(){
      const {serverId, legionId} = this.selectedChar;
      window.open(`https://aion.plaync.com/legions/server/${serverId}/id/${legionId}/home`);
    },

    getOriginServerId(name){
      const nm = name || this.selectedServer;
      return _.find(this.servers, n => n.type === nm).id;
    },
    openDatabase(id){
    ...
    addThisChar(){
      ...
      const {serverId, charId} = this.selectedChar;
      if(_.find(history, {serverId, charId}) == null){
    ...
    autoSelect(){
      ...
      // if(!(this.selectedChar && this.selectedChar.userid)){
        if(this.lastKeyword === this.suggest[0].charName){
